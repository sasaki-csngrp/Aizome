...
- `key_hash` にはSHA-256ハッシュのみ保存し、生のAPIキーはDBに保存しない
- `created_by` は発行した管理者の `users.id`（外部キー制約なし、管理者削除後もキーを追跡可能にするため）
- `last_used_at` でキーの最終利用日時を追跡できる
- キー形式: `aizome_` + ランダム32バイト（hex）= 全71文字
...
4. api_keys テーブルは独立しているため任意のタイミングで作成可能（アプリ起動時に自動作成もされる）
